      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2 lg:gap-3 w-full lg:w-auto">
          ...
          {viewMode === 'Bookings' ? (
            ...
            >
               <Printer className="w-4 h-4" /> <span className="hidden xs:inline">Export Manifest</span><span className="xs:hidden">Export</span>
            </button>
          ) : (
        ...
        <table className="w-full text-left text-[10px]">
           ...
           <tbody className="divide-y divide-slate-100">
             {filteredBookings.map(booking => (
               <tr key={booking.id}>
                 ...
                 <td className="py-3 text-right font-black">₹{booking.total}</td>
           ...
           <tfoot>
              <tr className="border-t-2 border-slate-900">
                 ...
                 <td className="py-4 text-right font-black text-sm">₹{filteredBookings.reduce((acc, curr) => acc + curr.total, 0).toLocaleString()}</td>
                 <td></td>
              </tr>
           </tfoot>
        </table>
